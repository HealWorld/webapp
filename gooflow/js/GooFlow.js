...
var endCnt = 0;
var existedWorkCnt = 0;
var execSQLCnt = 0;
var variableSetCnt = 0;
var exportTextCnt = 0;
var conditionJudgeCnt = 0;
var dataExtraCnt = 0;
var mailCnt = 0;
var reviewFileCnt = 0;
var storeProcessCnt = 0;
var waitCnt = 0;
var execJSCnt = 0;
...
function GooFlow(bgDiv, property) {
	...
	if(property.haveHead) {
		...
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='新建作业' title='新建作业'><i class='ico_" + property.headBtns[0] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='打开作业' title='打开作业'><i class='ico_" + property.headBtns[1] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='保存作业' title='保存作业'><i class='ico_" + property.headBtns[2] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='回撤步骤' title='回撤步骤'><i class='ico_" + property.headBtns[3] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='回复步骤' title='恢复步骤'><i class='ico_" + property.headBtns[4] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='刷新显示' title='刷新显示'><i class='ico_" + property.headBtns[5] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='执行作业' title='执行作业'><i class='ico_start" + "'></i></a>"
		tmp += "</div>";
		this.$head = $(tmp);
...
GooFlow.prototype = {
	...
	addNode: function(id, json, isLoadJsonData) {
		...
		if(nodeType == "existedWork") {
			json.name = "已有作业" + ++existedWorkCnt;
		}
		if(nodeType == "execSQL") {
			json.name = "执行SQL" + ++execSQLCnt;
		}
		if(nodeType == "variableSet") {
			json.name = "变量设置" + ++variableSetCnt;
		}
		if(nodeType == "exportText") {
			json.name = "导出文本" + ++exportTextCnt;
		}
		if(nodeType == "conditionJudge") {
			json.name = "条件判断" + ++conditionJudgeCnt;
		}
		if(nodeType == "dataExtra") {
			json.name = "数据抽取" + ++dataExtraCnt;
		}
		if(nodeType == "mail") {
			json.name = "邮件" + ++mailCnt;
		}
		if(nodeType == "reviewFile") {
			json.name = "检查文件" + ++reviewFileCnt;
		}
		if(nodeType == "storeProcess") {
			json.name = "存储过程" + ++storeProcessCnt;
		}
		if(nodeType == "wait") {
			json.name = "等待" + ++waitCnt;
		}
		if(nodeType == "execJS") {
			json.name = "执行JS文件" + ++execJSCnt;
		}
		...
		if(json.type.indexOf(" round") < 0) { //不是圆的
	...
	initWorkForNode: function() {
		...
		this.$workArea.delegate(".ico + td", "dblclick", {
			...
			if(nodeClassName.indexOf("existedWork") > 0) {
				$("#existedWorkDiag").find('input').eq(0).val(oldTxt);
				$("#existedWorkDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("Judge") > 0) {
				//				currentEditNodeID = $(this).parents(".GooFlow_item").attr("id");
				$("#conditionJudgeDiag").find('input').eq(0).val(oldTxt);
				$("#conditionJudgeDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("reviewFile") > 0) {
				$("#reviewFileDiag").find('input').eq(0).val(oldTxt);
				$("#reviewFileDiag").dialog('open');
				return;
			}
			/* 
			if(nodeClassName.indexOf("SQL") > 0) {
				$("#sqlDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("variable") > 0) {
				$("#variableSetDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("exportText") > 0) {
				$("#exportTextDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("wait") > 0) {
				$("#waitDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("mail") > 0) {
				$("#mailDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("dataExtra") > 0) {
				$("#dataExtraDiag").dialog('open');
				return;
			}

			if(nodeClassName.indexOf("storeProcess") > 0) {
				$("#storeProcessDiag").dialog('open');
				return;
			}
			if(nodeClassName.indexOf("execJS") > 0) {
				$("#execJSDiag").dialog('open');
				return;
			}
			*/
			$("#EditNodeDiag").find('input').eq(0).val("");
			$("#EditNodeDiag").find('input').eq(0).val(oldTxt);;
			$("#EditNodeDiag").dialog('open');
			return;
		});
